...
class FederatedFlow(FLSpec):
    def __init__(
        ...
        **kwargs,
        ...
        self.reference_optimizers = reference_optimizers 
    ...
    def audit(self):
        ...
        target_reference_models = [
            PytorchModelTensor(copy.deepcopy(model),nn.CrossEntropyLoss(),self.device) 
            for model in self.reference_models
        ]
        self.global_pm_info = ReferenceAuditor(
            target_model,target_reference_models,datasets,self.global_pm_info
        )
        ...
        print(
            f"debug:reference attack for the global model uses {time.time() - start_time}"
        )
        ...
        for idx, reference_model_global in enumerate(self.reference_models):
            ...
            reference_model.to(train_reference_device)
            reference_optimizer = default_optimizer(
                reference_model, optimizer_like=self.optimizers[self.input]
            ) 
            if self.round_num > 0:
                reference_optimizer.load_state_dict(
                    deepcopy(self.reference_optimizers[self.input][idx].state_dict())
                )
                optimizer_to_device(optimizer=reference_optimizer, device=train_reference_device)

            reference_train_losses = []
            reference_data_loader = torch.utils.data.DataLoader(
                self.reference_datasets[idx],batch_size=batch_size_train, shuffle=True
            )
            for batch_idx, (data, target) in enumerate(reference_data_loader):
                ...
                if batch_idx % log_interval == 0:
                    reference_train_losses.append(loss.item())
            
            print(f"reference model training loss: {np.mean(reference_train_losses)}")
            reference_model.to("cpu")
            self.reference_models[idx] = deepcopy(reference_model)
            tmp_opt = deepcopy(self.reference_optimizers[self.input][idx])
            tmp_opt.load_state_dict(reference_optimizer.state_dict())
            self.reference_optimizer.append(tmp_opt)
            torch.cuda.empty_cache()            
        ...
        delattr(self, "reference_datasets")
        self.next(self.join, exclude=["training_completed"])
    ...
    def join(self,inputs):
        ...
        self.reference_models = [
            FedAvg([input.reference_models[i].cpu() for input in inputs]) 
            for i in range(len(inputs[0].reference_models))
        ]
        self.reference_optimizers.update(
            {input.collaborator_name: input.reference_optimizer for input in inputs}
        )
        del inputs
...
if __name__ == '__main__':
    ...
    reference_optimizers = {
        collaborator.name: [
            default_optimizer(reference_model,optimizer_type=args.optimizer_type) 
